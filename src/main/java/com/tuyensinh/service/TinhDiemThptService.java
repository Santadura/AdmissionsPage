...
public class TinhDiemThptService {
    ...
    public Map<String, Object> tinhDiem(String maNganh,
        ...
        for (NganhToHop th : dsToHop) {
            ...
            item.put("xetNguongText", String.format("(%.2f + %.2f + %.2f + %.2f) = %.2f",
                    q1.diemQuyDoi, q2.diemQuyDoi, q3.diemQuyDoi, mucDiemUuTien, xetNguong));

            item.put("congThucToHop", String.format("[(%.2f x %.0f + %.2f x %.0f + %.2f x %.0f) / %.0f] x 3",
                    q1.diemQuyDoi, hs1, q2.diemQuyDoi, hs2, q3.diemQuyDoi, hs3, W));
            item.put("diemToHopText", String.format("%.3f", dthxt));
            ...
            item.put("diemXetTuyenText", String.format("%.3f + %.2f + %.2f = %.2f", dthgxt, diemCong, dut, dxt));
            item.put("tongDiem", dthxt);
